      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-2">
          {projects.map((project) => (
            <Link href={`/ProjectDetails/${project.slug}`} key={project.slug} className="group">
              <article className="overflow-hidden rounded-[2rem] border border-white/10 bg-[#0a0a0a] transition-all duration-500 hover:-translate-y-1 hover:border-white/20">
                <div className="relative aspect-4/3 overflow-hidden">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6 lg:p-8">
                    <p className="font-body text-sm text-[#999999]">{project.category}</p>
                    ...
                    <div className="mt-2 flex items-end justify-between gap-4">
                      ...
                      <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-white/20 text-white transition-all duration-300 group-hover:border-white group-hover:bg-white group-hover:text-black">
                        ↗
                      </span>
                    </div>
                  </div>
